...
5. **Flush on short-lived processes**: Scripts, tests, and some serverless handlers must `flush()` / `shutdown()` (or `forceFlush()` on the provider) so batches reach Latitude before the process exits.
...
Supported LLM instrumentation identifiers and matching client libraries are listed in each README (OpenAI, Anthropic, Bedrock, LangChain, and others).
