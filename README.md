...
| Skill                                           | Description                                                                                        |
| ----------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| [latitude-setup](skills/latitude-setup)         | Zero-account onboarding: bootstrap a temporary Latitude account, instrument, verify traces, build a first Artifact, claim. |
...
> **`latitude-setup` builds on two others.** It orchestrates `latitude-cli` (install + auth) and `latitude-telemetry` (instrumentation), and ends by building a first Artifact from its own bundled template. The `skills` CLI does not auto-install dependency skills, so add all three when you want the from-scratch onboarding flow:
>
> ```sh
...
> ```
>
> **`latitude-artifacts` is for the reports after that**: any question over a window, comparisons, refreshable dashboards. It reads Latitude through the MCP, the CLI, or an API key.
>
> ```sh
